...
export default function AcceleratorFormPage() {
    ...
    const growthStageOptions = [
        ...
        { id: 'Product-market fit', name: 'Product-market fit' },
    ...
    const [growthStage, setGrowthStage] = useState({ id: 'Please Select', name: 'Please Select' })
    const [pastFunding, setPastFunding] = useState('')
    const [futureFunding, setFutureFunding] = useState('')
    const [traction, setTraction] = useState('')
    const [incubatorExperience, setIncubatorExperience] = useState('')
    ...
    const submitForm = async (event) => {
        ...
        if (!firstName || !lastName || !email || !companyName || !phone || !website || !launchYear || !sector || !pitchDeck || !pitchLink || !problemStatement || !solutionProposition || !foundingTeam || !yourTeam || growthStage.id === 'Please Select' || !pastFunding || !futureFunding || !traction || !incubatorExperience) {
        ...
        formData.append('founding_team', foundingTeam)
        formData.append('team', yourTeam)
        formData.append('growth_stage', growthStage.id)
        formData.append('funding', pastFunding)
        formData.append('future_funding', futureFunding)
        formData.append('traction', traction)
        formData.append('incubator_accelerator_experience', incubatorExperience)
        ...
        try {
            ...
            if (data.errors) {
                data.errors.forEach((error) => {
                    ...
                    } else if (error.message.includes('fields.lastname')) {
                        toast.error('The last name field is invalid.')
                    } else if (error.message.includes('fields.email')) {
                        toast.error('The email field is invalid.')
                    } else if (error.message.includes('fields.company')) {
                        toast.error('The company name field is invalid.')
                    } else if (error.message.includes('fields.phone')) {
                        toast.error('The phone field is invalid.')
                    } else if (error.message.includes('fields.website')) {
                        toast.error('The company website field is invalid.')
                    } else if (error.message.includes('fields.start_date')) {
                        toast.error('The launch year field is invalid.')
                    } else if (error.message.includes('fields.industry')) {
                        toast.error('The sector field is invalid.')
                    } else if (error.message.includes('fields.documentation_files')) {
                        toast.error('The pitch deck field is invalid.')
                    } else if (error.message.includes('fields.hs_linkedin_url')) {
                        toast.error('The pitch link field is invalid.')
                    } else if (error.message.includes('fields.0-2/customer_pain_points___target_audience')) {
                        toast.error('The problem statement field is invalid.')
                    } else if (error.message.includes('fields.0-2/value_proposition')) {
                        toast.error('The solution proposition field is invalid.')
                    } else if (error.message.includes('fields.founding_team')) {
                        toast.error('The founding team field is invalid.')
                    } else if (error.message.includes('fields.team')) {
                        toast.error('The your team field is invalid.')
                    } else if (error.message.includes('fields.growth_stage')) {
                        toast.error('The growth stage field is invalid.')
                    } else if (error.message.includes('fields.funding')) {
                        toast.error('The past funding field is invalid.')
                    } else if (error.message.includes('fields.future_funding')) {
                        toast.error('The future funding field is invalid.')
                    } else if (error.message.includes('fields.traction')) {
                        toast.error('The traction field is invalid.')
                    } else if (error.message.includes('fields.incubator_accelerator_experience')) {
                        toast.error('The incubator experience field is invalid.')
                    }
                })
            }
            ...
            if (data.inlineMessage) {
                ...
                setFoundingTeam('')
                setYourTeam('')
                setGrowthStage({ id: 'Please Select', name: 'Please Select' })
                setPastFunding('')
                setFutureFunding('')
                setTraction('')
                setIncubatorExperience('')
                setGdprConsent(true)
            }
        } catch (error) {
    ...
    return (
        <main className="bg-cf-blue-1000">
            <section
                className="bg-cf-green-900 relative isolate flex items-center justify-center overflow-hidden py-36 pb-16 sm:pb-32 sm:pt-48">
                ...
                />
                <div
                    className="absolute right-0 top-0 -z-10 h-full w-full bg-[url('/noise-light.png')] bg-repeat mix-blend-overlay"></div>
            </section>

            <section className="relative bg-white pb-12 pt-60 ">
                <div
                    className="mx-auto -mt-96 flex w-full max-w-4xl flex-col items-center justify-center px-4 sm:-mt-[20rem] sm:px-8 lg:px-12">
                    <form
                    ...
                    >
                        ...
                        <div className="col-span-2">
                            <TextArea
                                ...
                                label="Founding Team*"
                                rows="6"
                                placeholder="Type founding team"
                                value={foundingTeam}
                                onChange={(e) => setFoundingTeam(e.target.value)}
                                description="List the (co-)founders and their roles, including LinkedIn profile URLs."
                            />
                        </div>
                        <div className="col-span-2">
                            <TextArea
                                ...
                                description="Describe the skills, experience and expertise that best position your team to solve this problem."
                            />
                        </div>
                        <div className="col-span-2">
                            <Select
                                ...
                                onChange={event => {
                                    setGrowthStage(event)
                                }}
                                description="What product stage are you at?"
                            />
                        </div>
                        <div className="col-span-2">
                            <TextArea
                                ...
                                label="Past Funding*"
                                rows="6"
                                placeholder="Type past funding"
                                value={pastFunding}
                                onChange={(e) => setPastFunding(e.target.value)}
                                description="Have you raised funding for this venture and if so, from which sources?"
                            />
                        </div>
                        <div className="col-span-2">
                            <TextArea
                                ...
                                rows="6"
                                placeholder="Type future funding"
                                value={futureFunding}
                                onChange={(e) => setFutureFunding(e.target.value)}
                                description="What is your future fundraising plan to support this venture?"
                            />
                        </div>
                        <div className="col-span-2">
                            <TextArea
                                ...
                                label="Traction*"
                                rows="6"
                                placeholder="Type traction"
                                value={traction}
                                onChange={(e) => setTraction(e.target.value)}
                                description="Have you achieved any market traction? (e.g., MRR, ARR, MAU, DAU, GMV, Daily Transactions, etc.)"
                            />
                        </div>
                        <div className="col-span-2">
                            <TextArea
                                ...
                                name="incubator-experience"
                                id="incubator-experience"
                                label="Incubator / Accelerator Experience*"
                                rows="6"
                                placeholder="Type incubator/ accelerator experience"
                                value={incubatorExperience}
                                onChange={(e) => setIncubatorExperience(e.target.value)}
                                description="Please specify any incubator or accelerator program(s) participated in."
                            />
                        </div>
                        <div className="col-span-2">
                            <Checkbox
